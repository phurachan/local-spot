import { HotelContent } from '~/server/models/CMS'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Find existing hotel
    const existingHotel = await HotelContent.findById(id)
    ...
    // Update hotel
    const updatedHotel = await HotelContent.findByIdAndUpdate(
      id,
      updateData,
      { new: true, runValidators: true }
    ).lean()

    return createSuccessResponse(updatedHotel, { responseType: API_RESPONSE_CODES.UPDATED })
